...
enc_df = temp.join(pd.DataFrame.sparse.from_spmatrix(mlb.fit_transform(temp.pop('genres')),
                                                     index = temp.index,
                                                     columns = mlb.classes_))

enc_df = temp.join(pd.DataFrame.sparse.from_spmatrix(mlb.fit_transform(temp.pop('cast')),
                                                     index = temp.index,
                                                     columns = mlb.classes_))

enc_df = temp.join(pd.DataFrame.sparse.from_spmatrix(mlb.fit_transform(temp.pop('crew')),
                                                     index = temp.index,
                                                     columns = mlb.classes_))
...
model = Sequential()
model.add(Dense(128, input_dim=U.shape[1], activation = 'sigmoid'))
